...
const gettodo = async(request,response)=>{
    ...
    if(_id){
        ...
        var Todos = await todo.find({taskName:data});  //find by name
    }
    else{
...
const addtodo = async(request,response)=>{      //in this we are adding new task 
    console.log(request.body)                  // async is to increase wait time
    var oneStudent = await todo.create(request.body)            //create library mein already stored h new task ko crate krne ke liye  // await jab tak wait krega isse phle vale steps complete nhi hote 
    return response.json({status:"task added", todo:oneStudent}) // task add hone ke baad output ae task added
                                    //todo:oneStudent jo hume data add kia h uski frontend pr hi vo sara data uski id and version bhi show ho
}
...
//findone se agr value nhi exist kregi to null dega
// find se agr value nhi exist kregi to empty 